...
qp = 0.2; % probability of production link per population
qc = 0.2; % probability of influence link per population
fdpl =0.1 ;%fraction of dp at the beginning
...
AHT = zeros(nMediator,nCellType,nSample);
...
CmpAHT = zeros(nCellType,nSample);
CmpBHT = zeros(nCellType,nSample);
CmpEHT = zeros(nCellType,nSample);
rintAT = zeros(nMediator,nCellType,nSample);
rintBT = zeros(nMediator,nCellType,nSample);
rintET = zeros(nMediator,nCellType,nSample);
% V0XT: list of all species that reached coexistence at the end of a run
% VOXT is a matrix with 1 for species that are present and 0 for the ones
% that are not
V0DT = zeros(3,nCellType,nSample); 
V0RT = zeros(3,nCellType,nSample);
V0HT = zeros(3,nCellType,nSample);

DAD = zeros(1,nSample); % d is depletable, r is reusable, h is hybrid
% ABE are distrubutions: E mostly pos., B: equal, E:
DBD = zeros(1,nSample);
DED = zeros(1,nSample);
DAR = zeros(1,nSample);
DBR = zeros(1,nSample);
...
for ns = 1:nSample,
    ...
    %% Simulating dynamincs Hybrid
    AH = A.*(rand(nMediator,nCellType)<fdpl);
    [NeAH,CmpAH] = WellmixedInteraction_DpMM_ExMT4(nRound,r0,cellRatioArray,rIntMatA,nInitialCell,kSatVector,AH,B,kSatLevel,extTh,dilTh,tauf,dtau);
    [NeBH,CmpBH] = WellmixedInteraction_DpMM_ExMT4(nRound,r0,cellRatioArray,rIntMatB,nInitialCell,kSatVector,AH,B,kSatLevel,extTh,dilTh,tauf,dtau);
    [NeEH,CmpEH] = WellmixedInteraction_DpMM_ExMT4(nRound,r0,cellRatioArray,rIntMatE,nInitialCell,kSatVector,AH,B,kSatLevel,extTh,dilTh,tauf,dtau);
    ...
    V0AH = zeros(1,nCellType);
    V0BH = zeros(1,nCellType);
    V0EH = zeros(1,nCellType);
    V0AH(NeAH) = 1;
    V0BH(NeBH) = 1;
    V0EH(NeEH) = 1;
    ...
    Cmp0AH = zeros(1,nCellType);
    Cmp0BH = zeros(1,nCellType);
    Cmp0EH = zeros(1,nCellType);
    Cmp0AH(NeAH) = CmpAH;
    Cmp0BH(NeBH) = CmpBH;
    Cmp0EH(NeEH) = CmpEH;
